    ...
    create or replace table `trangdang`.`sales_modeling`.`dim_customer`
...
LEFT JOIN `trangdang`.`sales_modeling_staging`.`stg_dim_customer_category` dcat 
ON dc.customer_category_key = dcat.customer_category_key
LEFT JOIN `trangdang`.`sales_modeling_staging`.`stg_dim_buying_group` db
ON dc.buying_group_key = db.buying_group_key
LEFT JOIN `trangdang`.`sales_modeling`.`dim_city`dim_deli
ON dc.delivery_city_key = dim_deli.city_key
LEFT JOIN `trangdang`.`sales_modeling`.`dim_city`dim_pst
ON dc.postal_city_key = dim_pst.city_key
LEFT JOIN `trangdang`.`sales_modeling`.`dim_person` dper
ON dc.primary_contact_person_key = dper.person_key
LEFT JOIN `trangdang`.`sales_modeling`.`dim_person` dpa
